...
var aggregateBookAndHighlightDetails = async () => {
  ...
  const resultingHighlights = books.reduce((highlights, book) => {
    const bookRelatedAnnotations = annotations.filter(
      (annotation) => annotation.ZANNOTATIONASSETID === book.ZASSETID
    );
...
var BackupHighlights = class {
    ...
    if (highlightsFolder) {
      ...
      if (highlightsFilesToBackup.length > 0) {
        ...
        for (const file of highlightsFilesToBackup) {
          const fileName = import_path2.default.basename(file);
          await this.vault.adapter.copy(file, import_path2.default.join(highlightsBackupFolder, fileName));
        }
  ...
  async backupSingleBookHighlights(filename) {
    const bookFilePathToBackup = import_path2.default.join(this.settings.highlightsFolder, `${filename}.md`);
    const vaultFile = this.vault.getFileByPath(bookFilePathToBackup);
    if (!vaultFile) {
      return;
    }
    const backupBookTitle = `${filename}-bk-${Date.now()}.md`;
    await this.vault.adapter.copy(vaultFile.path, import_path2.default.join(this.settings.highlightsFolder, backupBookTitle));
  }
};
...
var SaveHighlights = class {
  ...
  async saveAllBooksHighlightsToVault(highlights) {
    ...
    for (const combinedHighlight of highlights) {
      ...
      const filePath = import_path3.default.join(this.settings.highlightsFolder, `${renderedFilenameTemplate}.md`);
...
var checkBookExistence = async (item, vault, settings) => {
  const renderedFilename = await renderHighlightsTemplate(item, settings.filenameTemplate);
  const pathToBookFile = import_path4.default.join(settings.highlightsFolder, `${renderedFilename}.md`);
...
var OverwriteBookModal = class extends import_obsidian.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian.Setting(contentEl).addButton((YesButton) => {
      YesButton.setButtonText("Yes").setCta().onClick(async () => {
        bookToOverwrite ? await this.plugin.saveHighlights.saveSingleBookHighlightsToVault([bookToOverwrite], false) : await this.plugin.aggregateAndSaveHighlights();
        this.close();
      });
    }).addButton((NoButton) => {
...
var AppleBooksHighlightsImportPluginSettings = class {
  constructor() {
    ...
    this.filenameTemplate = `{{{${allowedFilenameTemplateVariables[0]}}}}`;
  }
};
var IBookHighlightsSettingTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian2.Setting(containerEl).setName("Backup highlights").setDesc(
      createFragment((el) => {
    ...
    const filenameTemplate = new import_obsidian2.Setting(containerEl).setName("Template for naming highlight files").setDesc(
      createFragment((el) => {
